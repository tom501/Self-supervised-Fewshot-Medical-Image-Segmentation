...
def superpix_vol(img, compact, method='felzenszwalb', **kwargs):
    ...
    if method=="felzenszwalb":
        ...
        for ii in range(img.shape[0]):
            out_vol[ii, ...] = seg_func(img[ii, ...], min_size=400, sigma=1, channel_axis=channel_axis)
    elif method=="felzenszwalb_3d": out_vol = felzenszwalb_3d(img, scale=1, sigma=1, min_size=400, spacing=(5.,0.44,0.44))
    elif method == "slic":
...
def superpix_wrapper(img, compact, verbose=False, fg_thresh=1e-4, method="felzenszwalb", stud=""):
...
for compact in compactness:
    MODE =""
    if "felzenszwalb" in DATASET_CONFIG[DOMAIN]["method"]: MODE = DATASET_CONFIG[DOMAIN]["method"]
    elif DATASET_CONFIG[DOMAIN]["method"]=="slic":
        add = "" if not slic_zero else "_SLICO"
        MODE = str(compact) + add  # minimum size of pesudolabels.
    class_flag = ""
    ...
    for pm in pmlist:
        ...
        if pm not in iou_dict.keys(): iou_dict[pm] = []
        if pm not in labels_dict.keys(): labels_dict[pm] = []
        ...
        for stud in studies:
            ...
            elif "SUS2020_PMs" in DOMAIN: timefold = glob.glob(f'{HOME}/SUS2020/PMs_niis/{stud}/*')
            ...
            for t, img_fid in enumerate(timefold):
                ...
                out_fg, out_seg, overlap_seg = superpix_wrapper(sitk.GetArrayFromImage(im_obj), compact=compact,
                                                                verbose=True, stud=stud,
                ...
                for i in range(loop):
                    ...
                    if "mask_fg" in DATASET_CONFIG[DOMAIN].keys():
                        ...
                        iou_dict[pm].append(v)
                        labels_dict[pm].append(np.sum(np.unique(overlap_seg_tmp)!=0))
    ...
    if "mask_fg" in DATASET_CONFIG[DOMAIN].keys():
        ...
        with open(labels_filename, "wb") as f: pkl.dump(labels_dict, f)

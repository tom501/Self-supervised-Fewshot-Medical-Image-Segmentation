...
def felzenszwalb_cython_3d(image, double scale=1, sigma=0.8, Py_ssize_t min_size=20, spacing=(1,1,1,1)):
    ...
    sigma = np.array([sigma,sigma,sigma,0], dtype=dtype)
    sigma /= spacing.astype(dtype)

    #image = ndi.gaussian_filter(image, sigma=sigma)
    height, width, depth = image.shape[:3]  # depth, height, width!
    #image = image[..., None]
...
#    cost1 = np.sqrt(np.sum((image[:, 1:, :] - image[:, :width-1, :])**2, axis=-1)  # y
#    cost2 = np.sqrt(np.sum((image[:, 1:, 1:] - image[:, :width-1, :depth-1])**2, axis=-1)) # yz
#    cost3 = np.sqrt(np.sum((image[:, :, 1:] - image[:, :, :depth-1])**2, axis=-1)) # z
#    cost7 = np.sqrt(np.sum((image[:, 1:, :depth-1] - image[:, :width-1, 1:])**2, axis=-1)) # yz
#    cost9 = np.sqrt(np.sum((image[1:, 1:, :] - image[:height-1, :width-1, :])**2, axis=-1)) # xy
#    cost10 = np.sqrt(np.sum((image[1:, 1:, 1:] - image[:height-1, :width-1, :depth-1])**2, axis=-1)) # xyz
#    cost11 = np.sqrt(np.sum((image[1:, :, 1:] - image[:height-1, :, :depth-1])**2, axis=-1)) # xz
#    cost12 = np.sqrt(np.sum((image[1:, :width-1, :] - image[:height-1, 1:, :])**2, axis=-1)) # xy
#    cost13 = np.sqrt(np.sum((image[1:, :width-1, :depth-1] - image[:height-1, 1:, 1:])**2, axis=-1)) # yz
#    cost14 = np.sqrt(np.sum((image[1:, :, :depth-1] - image[:height-1, :, 1:])**2, axis=-1)) # xz
#    cost15 = np.sqrt(np.sum((image[1:, 1:, :depth-1] - image[:height-1, :width-1, 1:])**2, axis=-1)) # xyz
#    cost16 = np.sqrt(np.sum((image[1:, :width-1, 1:] - image[:height-1, 1:, :depth-1])**2, axis=-1)) # xyz
#    cost25 = np.sqrt(np.sum((image[1:, :, :] - image[:height-1, :, :])**2, axis=-1)) # x

#    # assuming spacing is equal in xy dir.
#    cost1 = np.sqrt(1*np.sum((image[:, 1:, :] - image[:, :width-1, :])**2, axis=-1))  # y
#    cost2 = np.sqrt((5**2)*np.sum((image[:, 1:, 1:] - image[:, :width-1, :depth-1])**2, axis=-1)) # yz
#    cost3 = np.sqrt(7**2*np.sum((image[:, :, 1:] - image[:, :, :depth-1])**2, axis=-1)) # z
#    cost7 = np.sqrt(5**2*np.sum((image[:, 1:, :depth-1] - image[:, :width-1, 1:])**2, axis=-1)) # yz
#    cost9 = np.sqrt(1*np.sum((image[1:, 1:, :] - image[:height-1, :width-1, :])**2, axis=-1)) # xy
#    cost10 = np.sqrt(17*np.sum((image[1:, 1:, 1:] - image[:height-1, :width-1, :depth-1])**2, axis=-1)) # xyz
#    cost11 = np.sqrt(5**2*np.sum((image[1:, :, 1:] - image[:height-1, :, :depth-1])**2, axis=-1)) # xz
#    cost12 = np.sqrt(1*np.sum((image[1:, :width-1, :] - image[:height-1, 1:, :])**2, axis=-1)) # xy
#    cost13 = np.sqrt(5**2*np.sum((image[1:, :width-1, :depth-1] - image[:height-1, 1:, 1:])**2, axis=-1)) # yz
#    cost14 = np.sqrt(5**2*np.sum((image[1:, :, :depth-1] - image[:height-1, :, 1:])**2, axis=-1)) # xz
#    cost15 = np.sqrt(17*np.sum((image[1:, 1:, :depth-1] - image[:height-1, :width-1, 1:])**2, axis=-1)) # xyz
#    cost16 = np.sqrt(17*np.sum((image[1:, :width-1, 1:] - image[:height-1, 1:, :depth-1])**2, axis=-1)) # xyz
#    cost25 = np.sqrt(1*np.sum((image[1:, :, :] - image[:height-1, :, :])**2, axis=-1)) # x

    # assuming spacing is equal in xy dir.
    assert spacing[1]==spacing[2], "The x-y spacing is not equal."
